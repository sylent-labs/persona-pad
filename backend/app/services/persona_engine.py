...
_BAN_REINFORCEMENT = (
    ...
    "- When the other side puts forward a compensation number that is too low to accept "
    "(a lowball they offered, not a question asking Van Keith to name a number), do not "
    "mention it in the reply at all. No confirming it, no declining it, no countering it, "
    "no 'I'd like to discuss the rate', no 'the rate doesn't align', no 'that doesn't "
    "work', no reference to rate, pay, comp, compensation, salary, hourly, W2, or the "
    "dollar figure. The words and the number simply do not appear. Answer everything else "
    "in their message and leave the offer unaddressed on purpose.\n"
